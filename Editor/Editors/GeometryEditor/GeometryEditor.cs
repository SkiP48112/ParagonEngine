...
namespace Editor.Editors
{
    ...
    //      renderer, this class and the WPF viewer will become deprecated.
    class MeshRenderedVertexData : ViewModelBase
    {
    ...
    {
        public ObservableCollection<MeshRenderedVertexData> Meshes { get; } = new ObservableCollection<MeshRenderedVertexData>();
        ...
        {
            Debug.Assert(lod?.Meshes.Any() == true, "Can't create MeshRenderer for lod with no meshes");
            ...
            // In order to set up camera position to target properly we need to know it's bounding box.
            ...
            // We need this part to unpack the packed normals. Look at geomPackVerticesStatic()
            var intervals = 2.0f / ((1 << 16) - 1);
            foreach (var mesh in lod.Meshes)
            {
                var vertexData = new MeshRenderedVertexData();
